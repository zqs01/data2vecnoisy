...
from ..data.raw_audio_dataset import BinarizedAudioDatasetNoisy, FileAudioDatasetNoisy
...
class AudioPretrainingConfig(FairseqDataclass):
    ...
    noise_data_path: Optional[str] = field(
        default=None,
        metadata={"help": "noise data manifest path"},
    )

    noise_snr: Optional[str] = field(default='0', metadata={'help': 'noise SNR in audio'})


@register_task("audio_pretrainingnoisy", dataclass=AudioPretrainingConfig)
class AudioPretrainingnoisyTask(FairseqTask):
    """ """
    ...
    def load_dataset(self, split: str, task_cfg: FairseqDataclass = None, **kwargs):
        ...
        if getattr(task_cfg, "binarized_dataset", False):
            self.datasets[split] = BinarizedAudioDatasetNoisy(
                data_path,
                split=split,
            ...
            noise_path = os.path.join(self.cfg.noise_data_path, "{}.tsv".format(split))


            self.datasets[split] = FileAudioDatasetNoisy(
